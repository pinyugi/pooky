...
function isTamperingCheck(path, funcName){
	return (
		...
		(path.get("right").type = "MemberExpression" ||
		path.get("right.value").node == "function" )
	);
...
function isEvalSourceCode(path, funcName){
	...
	return (
		...
		(path.get("callee.property").type == "MemberExpression" ||
			(
				path.get("callee.property").type == "Literal" && 
				path.get("callee.property.value").node == "toString"
			)
		)
	);
}

function removeTamperingChecks(fileName, newFileName, addLocalStorage=false){
		...
		traverse(ast, {
			"BinaryExpression"(path){
				if(isTamperingCheck(path, funcName)){
					...
					parent.replaceWith(
						t.variableDeclaration("var",[t.variableDeclarator(id, t.booleanLiteral(true))])
					);
				}
			}
		});
					...
					if(addLocalStorage){
						...
						parent.insertAfter(
							t.expressionStatement(
								t.callExpression(
									t.memberExpression(t.identifier("localStorage"), t.identifier("setItem")),
									[t.stringLiteral(evaluatedId), t.identifier(evaluatedId)]
								)
							)
						);
					}
